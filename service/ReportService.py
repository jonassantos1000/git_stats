...
class ReportService:
    ...
    def gerar_relatorio(self, token, id_usuario):
        ...
        commit_ordenados = sorted(self.__buscar_commits(), key=lambda x: self.__formatar_data(x.get("created_at", "")))
        ...
        for commit in commit_ordenados:
            ...
            lista_commit_detalhes.append(
                self.__buscar_detalhes_commit(commit.get("project_id"), commit.get("push_data").get("commit_to"))
            )
    ...
    def __buscar_commits(self):
        ...
        while True:
            params = {
                ...
                "after": (self.data_atual - timedelta(days=8)).strftime("%Y-%m-%d"),
                "before": (self.data_atual + timedelta(days=1)).strftime("%Y-%m-%d"),
                "per_page": "100",
                "page": {pagina}
            }
